...
AceLexer = (function lexer_init() {
    ...
    function makeRegexSwitch(patterns, flags) {
        ...
        var numGroups = map(patternStrings, function (p) {
            var count = 0;
            p.replace(parenFindingRegex, function (full, paren, offset) {
                if (paren) count++;
            });
            return count;
        });
        ...
        var caseGroupNums = [];
        var idx = 1;
        forEach(numGroups, function (n) {
            caseGroupNums.push(idx);
            idx += n + 1;
        });
    ...
    function makeTokenProducer(regexData, flags) {
        ...
        forEach(sortedStates, function(state) {
            ...
            data[state] = {
                switcher: makeRegexSwitch(map(procCases, function(x) {
                    return x[0];
                }), flags),
                tokenTypes: map(procCases, function(x) {
                    return x[1];
                }),
    ...
    var jsTokenProducer = makeTokenProducer(
    {
        ...
        'csscommon': [
            // common outside of style rule brackets
            {include:'common'},
    ...
    function makeIncrementalLexer(tokenProducer) {
        ...
        function updateBuffer(newBuffer, spliceStart, charsRemoved, charsAdded) {
            ...
            forEach(dirtyTokens, function (t) {
                dirtyTokenKeys.push(t.key);
            });
            dirtyTokenKeys = filter(dirtyTokenKeys, function (k) {
                return tokens.containsKey(k);
            });
